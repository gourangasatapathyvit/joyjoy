...
📖 **Docs:** [`ARCHITECTURE.md`](./ARCHITECTURE.md) (system overview) · [`CLAUDE.md`](./CLAUDE.md) (contributor/agent guide) · [`backend/README.md`](./backend/README.md) · [`frontend/README.md`](./frontend/README.md) · [`PLAN.md`](./PLAN.md).

## Layout
- `backend/` — FastAPI + deepagents engine (also serves the SPA). `app/db/` (13 SQLAlchemy models, async engine, Fernet, seeds), `app/dbfs.py` (DB→agent bridge), `app/sandbox*.py` (OpenSandbox), `alembic/` (migrations). See [backend/README.md](./backend/README.md).
...
- `scripts/` — `start_all.sh` (one-command full stack), `serve.sh` (build SPA + run backend), `restart_backend.sh`, `validate_models.py`.
- `data/` — dev SQLite DBs + per-user workspace files (gitignored).
- `docs/branding/` — brand kit: logos, favicons.
...
(skins, providers, base models, global MCP servers, global skills). When not
...
`JWT_SECRET`). The app DB and the LangGraph checkpointer share one Postgres database
via disjoint tables.
...
include the OpenSandbox execution layer — use `scripts/start_all.sh` for that. See
[ARCHITECTURE.md](./ARCHITECTURE.md) for the full deployment/data picture.
