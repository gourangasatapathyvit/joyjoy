...
- **Model providers** (LangChain SDKs; dispatched by `provider` in each model spec): Azure OpenAI, Anthropic (incl. Azure AI Foundry `/anthropic` Claude endpoint), AWS Bedrock (`langchain-aws`/boto3), Google GenAI, NVIDIA NIM (`langchain-nvidia-ai-endpoints`'s `ChatNVIDIA`), xAI/Grok (`langchain-xai`'s `ChatXAI`, API key **or** OAuth device-code login — see below), and any OpenAI-compatible endpoint (OpenAI itself, OpenRouter, DeepSeek, Groq, local servers — via a configurable base URL). Catalog = `global_models` + per-user `user_models`; keys referenced as `${VAR}` and expanded at build (kept out of the DB seed). **Policy**: a provider with its own official LangChain package gets a dedicated `Provider` + `build_model_for` branch (NVIDIA and xAI) rather than being folded into the generic OpenAI-compatible bucket — the dedicated package's own capability metadata beats hand-verifying via id heuristics or live trial-and-error. xAI and its OAuth variant are opt-in only (never seeded as a `global_model`) and reasoning-effort support is model-id-gated, since sending it to an unsupported Grok model is a hard 400, not a silent no-op.
- **xAI OAuth (device-code grant)** (`app/agent/xai_oauth.py`): lets a user attach a SuperGrok / X Premium+ subscription instead of a metered API key. RFC 8628 Device Authorization Grant against `auth.x.ai` — discovery → device code shown to the user → poll → access + refresh token — with the access token used **directly** as the xAI API bearer (no separate token exchange). xAI rotates the refresh token on every use, so `_persist_xai_oauth_tokens` serializes concurrent refreshes for the same model behind a per-model `asyncio.Lock`, and `_get_or_build` checks/refreshes the token on *every* agent-cache lookup (not just a cold build) since the cache itself has no TTL to force a periodic recheck.
- **Dynamic model discovery**: instead of hand-typing a model id, the Providers tab can call `POST /v1/models/config/discover` with the entered credentials to list a provider's live catalog (`agent.py`'s per-provider adapters — `httpx` for OpenAI/Gemini/Anthropic/Azure REST list-models calls, `boto3` for Bedrock's `list_foundation_models`, `ChatNVIDIA.get_available_models()` for NVIDIA, `_discover_xai` for both `xai` and `xai_oauth`), then bulk-save a multi-selected subset via `POST /v1/models/config/save-bulk`. NVIDIA's adapter is the one with REAL per-model capability flags (`supports_tools`/`supports_thinking`/`supports_structured_output`) rather than guessing from the model id string like the others still do. Per-user model ids are **provider-qualified composites** (`{provider}:{raw_id}`, e.g. `openai:gpt-4.1`) so the same base model name can exist under different providers without colliding with a global model or another user model — global (seeded) ids stay bare. Discovery re-uses a model's own stored (decrypted) credentials when editing, so switching a model's underlying deployment doesn't require retyping the API key.
