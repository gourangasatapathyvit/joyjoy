...
def _run_gateway_runs_api_streaming(
    ...
    *, put_gateway_event, cancel_event,
    attachments=None, cfg=None, username=None,
):
    ...
    if username:
        headers["X-User-Id"] = username  # joyjoy: per-user backend isolation
        ...
        for raw_line in resp:
            ...
            if payload_event == "approval.request":
                ...
                if approval_data:
                    ...
                    # joyjoy: mirror into the WebUI pending-approval queue so the card
                    # buttons enable and /api/approval/respond can target this approval.
                    try:
                        from api.route_approvals import submit_pending as _submit_pending
                        _submit_pending(session_id, approval_data)
                    except Exception:
                        logger.debug("failed to mirror runs approval into pending queue", exc_info=True)
