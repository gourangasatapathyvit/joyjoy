...
# ── joyjoy multi-user: token -> username (parallel store, backward-compatible) ─
_SESSION_USERS_FILE = STATE_DIR / '.session_users.json'


def _load_session_users() -> dict[str, str]:
    try:
        if _SESSION_USERS_FILE.exists():
            data = json.loads(_SESSION_USERS_FILE.read_text(encoding='utf-8'))
            if isinstance(data, dict):
                return {t: u for t, u in data.items() if isinstance(t, str) and isinstance(u, str)}
    except Exception as e:
        logger.debug("Failed to load session users, starting fresh: %s", e)
    return {}


def _save_session_users(mapping: dict[str, str]) -> None:
    try:
        STATE_DIR.mkdir(parents=True, exist_ok=True)
        fd, tmp = tempfile.mkstemp(dir=STATE_DIR, suffix='.session_users.tmp')
        try:
            with os.fdopen(fd, 'w', encoding='utf-8') as f:
                json.dump(mapping, f)
            os.chmod(tmp, 0o600)
            os.replace(tmp, _SESSION_USERS_FILE)
        except Exception:
            try:
                os.unlink(tmp)
            except OSError:
        ...
        logger.debug("Failed to persist session users: %s", e)


_session_users = _load_session_users()
...
def is_auth_enabled() -> bool:
    """True if password, passkey, or multi-user account auth is configured."""
    if is_password_auth_enabled() or are_passkeys_enabled():
        return True
    # joyjoy: defining accounts (users.json) also turns auth on, so the login
    # gate applies even when no single HERMES_WEBUI_PASSWORD is set.
    try:
        from api.users import multi_user_enabled
...
def create_session(username: str | None = None) -> str:
    ...
    joyjoy: when *username* is given, bind the session token to that user so
    ``session_username()`` can resolve the authenticated identity later (used to
    forward ``X-User-Id`` to the agent backend for per-user isolation). Omitting
    it preserves the legacy anonymous single-password session.
    """
    ...
    if username:
        with _SESSION_USERS_LOCK:
            ...
            _save_session_users(_session_users)
...
def session_username(cookie_value: str) -> str | None:
    """joyjoy: return the username bound to a *verified* session, or None.

    Returns None for anonymous (single-password) sessions created without a
    username, so callers fall back to legacy behaviour.
    """
