...
<div class="card">
  ...
  <form id="login-form" data-invalid-pw="{{LOGIN_INVALID_PW}}" data-conn-failed="{{LOGIN_CONN_FAILED}}">
    <input type="text" id="username" placeholder="Username" autocomplete="username" autocapitalize="none" autocorrect="off" spellcheck="false" style="display:none">
...
<head>
  ...
  <title>joyjoy is restarting</title>
</head>
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path in ("/", "/index.html") or parsed.path.startswith("/session/"):
        try:
            ...
            version_token = _static_cache_token()
    ...
    if parsed.path == "/api/auth/status":
        ...
        try:
            from api.users import multi_user_enabled as _multi_user_enabled
    ...
    if parsed.path == "/api/models":
        # joyjoy: in gateway mode, surface the backend gateway's configured models
        # (so the picker shows exactly what the deepagent backend supports). Falls
        # back to the local catalog on any error.
        try:
            from api.gateway_chat import _gateway_api_key, _gateway_base_url, webui_gateway_chat_enabled
            if webui_gateway_chat_enabled(get_config()):
                import urllib.request as _u
                _base = _gateway_base_url(get_config()).rstrip("/")
                _key = _gateway_api_key()
                _hdr = {"Authorization": f"Bearer {_key}"} if _key else {}
                try:  # forward the logged-in user so the picker shows their per-user models
                    from api.auth import parse_cookie, session_username
                    _un = session_username(parse_cookie(handler))
                    if _un:
                        _hdr["X-User-Id"] = _un
                except Exception:
                    pass
                with _u.urlopen(_u.Request(_base + "/v1/models", headers=_hdr), timeout=5) as _r:
                    _items = [m for m in (json.loads(_r.read()).get("data") or []) if m.get("id")]
                if _items:
                    # Group by provider with friendly labels so the picker shows
                    # which provider each model belongs to (optgroup per provider).
                    _plabels = {
                    ...
                    for _p in _order + [p for p in _byprov if p not in _order]:
                        if _byprov.get(_p):
                            _groups.append({"provider": _plabels.get(_p, _p), "provider_id": _p, "models": _byprov[_p]})
                    # active_provider=None: the grouped catalog is complete, so skip
                    # the redundant live-model fetch (which would add a stray group).
                    return j(handler, {
                        "active_provider": None,
                        "default_model": _items[0]["id"],
                        "groups": _groups,
                    })
        except Exception:
            logger.debug("gateway /v1/models fetch failed; using local catalog", exc_info=True)
    ...
    if parsed.path == "/api/models/live":
        # joyjoy: in gateway mode the backend gateway is the single source of
        # truth for models. Return its catalog here too (same {"models":[{id,label}]}
        # shape the frontend _addLiveModelsToSelect expects) so the live-fetch
        # never falls back to the local static provider catalog — which still
        # lists Hermes-era models like claude-opus-4-7 and would append them to
        # the picker, and is unreachable anyway now that the bundled hermes-agent
        # is removed. Returned ids dedup against the static joyjoy optgroup.
        try:
            from api.gateway_chat import _gateway_api_key, _gateway_base_url, webui_gateway_chat_enabled
            if webui_gateway_chat_enabled(get_config()):
                import urllib.request as _u
                ...
                _key = _gateway_api_key()
                _hdr = {"Authorization": f"Bearer {_key}"} if _key else {}
                try:  # forward the logged-in user so live models match their catalog
                    from api.auth import parse_cookie, session_username
                    _un = session_username(parse_cookie(handler))
                    if _un:
                        _hdr["X-User-Id"] = _un
                    ...
                    pass
                with _u.urlopen(_u.Request(_base + "/v1/models", headers=_hdr), timeout=5) as _r:
                    _ids = [m.get("id") for m in (json.loads(_r.read()).get("data") or []) if m.get("id")]
                return j(handler, {"models": [{"id": i, "label": i} for i in _ids]})
        except Exception:
            logger.debug("gateway /v1/models (live) fetch failed; using local catalog", exc_info=True)
    ...
    if parsed.path == "/api/sessions":
        ...
        try:
            ...
            resp = _session_list_payload_to_response(payload)
            # joyjoy: per-user sidebar scoping — when multi-user is on, show only
            # the requesting user's own sessions (owned via chat-start stamping).
            try:
                from api.auth import parse_cookie, session_username
                from api import users as _users
                cur = session_username(parse_cookie(handler))
                if cur and _users.multi_user_enabled():
                    owners = _users.session_owners()
                    resp["sessions"] = [
                        srow for srow in resp.get("sessions", [])
                        if owners.get(str(srow.get("session_id"))) == cur
                    ]
            except Exception:
                logger.debug("per-user session scoping skipped", exc_info=True)
            return j(handler, resp, pretty=False)
        finally:
            diag.finish()
    ...
    if parsed.path == "/api/skills":
        ...
        # joyjoy: in gateway mode, list the deepagent backend's global (read-only) +
        # per-user skills so the Skills tab reflects what the agent actually loads.
        _gw = _proxy_gateway_get(handler, "/v1/skills")
        if isinstance(_gw, dict) and isinstance(_gw.get("skills"), list):
            return j(handler, {"skills": _gw["skills"]})
    ...
    if parsed.path == "/api/skills/content":
        # joyjoy: in gateway mode the deepagent backend owns skills — proxy the
        # read-only view (forwarding name+file). Authoritative when reachable.
        _gw = _proxy_gateway_get(handler, "/v1/skills/content?" + (parsed.query or ""))
    ...
    # ── Memory API (GET) ── joyjoy: per-user notes/profile/soul from the gateway.
    if parsed.path == "/api/memory":
        _gw = _proxy_gateway_get(handler, "/v1/memory")
        if isinstance(_gw, dict) and any(k in _gw for k in ("memory", "user", "soul")):
            return j(handler, _gw)
    ...
    if parsed.path == "/api/mcp/servers":
        # joyjoy: in gateway mode, surface the deepagent backend's global+user MCP
        # servers (per calling user). Falls back to local Hermes config on failure.
        _gw = _proxy_gateway_get(handler, "/v1/mcp/servers")
        if isinstance(_gw, dict) and isinstance(_gw.get("servers"), list):
            return j(handler, _gw)
        return _handle_mcp_servers_list(handler)

    # ── MCP Tools (GET) ──
    if parsed.path == "/api/mcp/tools":
        _gw = _proxy_gateway_get(handler, "/v1/mcp/tools")
        if isinstance(_gw, dict) and isinstance(_gw.get("tools"), list):
            return j(handler, _gw)
        return _handle_mcp_tools_list(handler)

    # ── Models / Providers config (GET) ── joyjoy: global (read-only) + per-user
    # models for the Settings→Providers tab, plus the provider field-schema.
    if parsed.path == "/api/models/config":
        _gw = _proxy_gateway_get(handler, "/v1/models/config")
        if isinstance(_gw, dict) and isinstance(_gw.get("models"), list):
            return j(handler, _gw)
        return j(handler, {"models": [], "providers": []})
...
def handle_post(handler, parsed) -> bool:
    ...
    # ── Skills (POST) ── joyjoy: user-skill CRUD goes to the gateway (per-user store);
    # global skills are rejected there as read-only. Falls back to Hermes if unreachable.
    if parsed.path == "/api/skills/save":
        _gw = _proxy_gateway_send(handler, "POST", "/v1/skills/save", body)
        if isinstance(_gw, dict):
            return j(handler, _gw)
        return _handle_skill_save(handler, body)

    if parsed.path == "/api/skills/delete":
        _gw = _proxy_gateway_send(handler, "POST", "/v1/skills/delete", body)
        if isinstance(_gw, dict):
            return j(handler, _gw)
        return _handle_skill_delete(handler, body)

    if parsed.path == "/api/skills/toggle":
        _gw = _proxy_gateway_send(handler, "POST", "/v1/skills/toggle", body)
        if isinstance(_gw, dict):
            return j(handler, _gw)
        return _handle_skill_toggle(handler, body)

    # ── Memory (POST) ── per-user notes/profile/soul via the gateway store.
    if parsed.path == "/api/memory/write":
        _gw = _proxy_gateway_send(handler, "POST", "/v1/memory/write", body)
        if isinstance(_gw, dict):
            return j(handler, _gw)
        return _handle_memory_write(handler, body)

    # ── Models / Providers CRUD (POST) ── joyjoy-only: per-user model add/edit/delete
    # via the gateway store; global models are rejected there as read-only.
    if parsed.path == "/api/models/config/save":
        _gw = _proxy_gateway_send(handler, "POST", "/v1/models/config/save", body)
        if isinstance(_gw, dict):
            return j(handler, _gw)
    ...
    if parsed.path == "/api/auth/login":
        ...
        # joyjoy: multi-user mode (users.json present) requires username+password
        # and binds the session to that username; otherwise legacy single-password.
        if _users.multi_user_enabled():
            if not username or not _users.verify_user(username, password):
                _record_login_attempt(client_ip)
                return bad(handler, "Invalid username or password", 401)
            session_user = _users.normalize(username)
        else:
        ...
        cookie_val = create_session(username=session_user)
...
def handle_patch(handler, parsed) -> bool:
    ...
    if parsed.path.startswith("/api/mcp/servers/"):
        ...
        _gw = _proxy_gateway_send(handler, "PATCH", "/v1/mcp/servers/" + name, body)
        if isinstance(_gw, dict):
            return j(handler, _gw)
...
def handle_delete(handler, parsed) -> bool:
    ...
    if parsed.path.startswith("/api/mcp/servers/"):
        name = parsed.path[len("/api/mcp/servers/"):]
        _gw = _proxy_gateway_send(handler, "DELETE", "/v1/mcp/servers/" + name, None)
        if isinstance(_gw, dict):
            return j(handler, _gw)
...
def handle_put(handler, parsed) -> bool:
    ...
    if parsed.path.startswith("/api/mcp/servers/"):
        name = parsed.path[len("/api/mcp/servers/"):]
        _gw = _proxy_gateway_send(handler, "PUT", "/v1/mcp/servers/" + name, body)
        if isinstance(_gw, dict):
            return j(handler, _gw)
...
def _start_chat_stream_for_session(
    ...
    *,
    ...
    if backend_is_gateway:
        # joyjoy: forward the authenticated user for per-user backend isolation
        worker_kwargs["username"] = getattr(s, "_joyjoy_user", None)
    else:
...
def _handle_chat_start(handler, body, diag=None):
    try:
        ...
        # joyjoy: bind the authenticated user to this turn so the gateway worker
        # forwards it to the backend as X-User-Id (per-user isolation).
        try:
            from api.auth import parse_cookie, session_username
            s._joyjoy_user = session_username(parse_cookie(handler))
            if s._joyjoy_user:
                from api import users as _users
...
def _proxy_gateway_get(handler, gateway_path: str):
    """In gateway mode, GET ``gateway_path`` from the deepagent backend, forwarding
    the logged-in user as ``X-User-Id`` (per-user isolation). Returns the parsed
    JSON dict, or ``None`` so callers can fall back to the local Hermes handler.
    """
    try:
        from api.gateway_chat import _gateway_api_key, _gateway_base_url, webui_gateway_chat_enabled
        ...
        base = _gateway_base_url(get_config()).rstrip("/")
        headers = {}
        ...
        try:
            ...
            uname = session_username(parse_cookie(handler))
            if uname:
                headers["X-User-Id"] = uname
        except Exception:
            logger.debug("could not resolve session user for gateway proxy", exc_info=True)
        with _u.urlopen(_u.Request(base + gateway_path, headers=headers), timeout=8) as r:
            return json.loads(r.read())
    except Exception:
        logger.debug("gateway GET %s failed; using local handler", gateway_path, exc_info=True)
        return None


def _proxy_gateway_send(handler, method: str, gateway_path: str, body):
    """In gateway mode, send a write (POST/PUT/PATCH/DELETE) to the deepagent backend,
    forwarding the logged-in user as ``X-User-Id``. Returns the parsed JSON dict, or
    ``None`` so callers fall back to the local Hermes handler.
    """
    try:
        from api.gateway_chat import _gateway_api_key, _gateway_base_url, webui_gateway_chat_enabled
        ...
        try:
            ...
            logger.debug("could not resolve session user for gateway write", exc_info=True)
        req = _u.Request(base + gateway_path, data=data, headers=headers, method=method)
        with _u.urlopen(req, timeout=15) as r:
            return json.loads(r.read())
    except Exception:
        logger.debug("gateway %s %s failed; using local handler", method, gateway_path, exc_info=True)
        return None


def _static_cache_token() -> str:
    """Cache-busting token for versioned assets + the service-worker cache name.

    In a real deploy ``WEBUI_VERSION`` is a git/release string that changes per
    deploy. Running joyjoy from source it is ``"unknown"`` (static), which would
    pin the SW cache forever and serve stale JS/CSS after edits. In that case,
    fold in the newest static-file mtime so the token changes on every edit and
    the service worker refreshes automatically (no manual SW unregister needed).
    """
    from urllib.parse import quote
    ...
    if base in ("", "unknown") or base.endswith("dev"):
        try:
            static_root = Path(__file__).parent.parent / "static"
            latest = 0.0
            for p in static_root.glob("*.*"):
                if p.suffix in (".js", ".css", ".html"):
                    latest = max(latest, p.stat().st_mtime)
            if latest:
                base = f"{base}-{int(latest)}"
        except Exception:
            logger.debug("static cache token mtime scan failed", exc_info=True)
