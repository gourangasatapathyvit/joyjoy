...
function _reconcileModelDropdownSelection(sel,data,previousState,opts){
  ...
  // joyjoy: if a saved session / persisted model is no longer offered by the
  // backend catalog, fall back to the configured default instead of resurrecting
  // a phantom model (e.g. a stale 'claude-opus-4-7' from a pre-migration chat).
  const _modelInCatalog = function(modelId){
    const mid=String(modelId||'').trim();
    if(!mid) return false;
    for(const g of ((data&&Array.isArray(data.groups))?data.groups:[])){
      for(const m of [...(g.models||[]),...(g.extra_models||[])]){
        if(m&&String(m.id).trim()===mid) return true;
      }
    }
    return false;
  };
  const _applyPreferringDefault = function(modelId, providerId){
    if(data&&data.default_model&&_modelInCatalog(data.default_model)&&!_modelInCatalog(modelId)){
      return _applyOrEnsure(data.default_model, data.active_provider||null);
    }
    return _applyOrEnsure(modelId, providerId);
  };
  if(activeSession&&activeSession.model){
    return _applyPreferringDefault(activeSession.model, activeSession.model_provider||null);
  }
  if(previousState&&previousState.model){
    return _applyPreferringDefault(previousState.model, previousState.model_provider||null);
  }
  return null;
}
...
function _assistantRoleHtml(tsTitle='', tpsText=''){
  ...
  return `<div class="msg-role assistant" ${tsTitle?`title="${esc(tsTitle)}"`:''}><div class="role-icon assistant" style="background:none;border:none;padding:0;overflow:hidden;"><img src="static/joyjoy-icon.svg" alt="" style="width:100%;height:100%;border-radius:50%;display:block"></div><span style="font-size:12px">${esc(_bn)}</span>${tps}</div>`;
}
