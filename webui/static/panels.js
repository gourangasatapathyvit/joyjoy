...
async function switchPanel(name, opts = {}) {
  ...
  if (nextPanel === 'mcp') { loadMcpServers(); loadMcpTools(); }
...
function renderSkills(skills) {
  ...
  for (const [cat, items] of Object.entries(cats).sort()) {
    ...
    for (const skill of items.sort((a,b) => a.name.localeCompare(b.name))) {
      const editable = skill.editable !== false;  // global skills are read-only
      const isDisabled = skill.disabled === true || skill.enabled === false;
      ...
      if (editable) {
        ...
        toggle.textContent = '🔒';
        toggle.title = 'Global skill (read-only)';
        toggle.style.cssText = 'opacity:.55;cursor:default';
      }
...
function _setSkillHeaderButtons(mode) {
  ...
  const hide = b => b && (b.style.display = 'none');
  if (mode === 'read') {
    const editable = !_currentSkillDetail || _currentSkillDetail.editable !== false;  // global = read-only
    (editable ? show : hide)(editBtn); (editable ? show : hide)(delBtn); hide(cancelBtn); hide(saveBtn);
  }
...
async function openSkill(name, el) {
  ...
  try {
    ...
    _currentSkillDetail = { name, content: data.content || '', linked_files: data.linked_files || {}, editable: data.editable !== false };
...
async function loadSettingsPanel(){
  try{
    ...
    const botNameField=$('settingsBotName');
    if(botNameField){
      botNameField.value=settings.bot_name||'joyjoy';
...
// joyjoy: the Providers tab manages the deepagent backend's model catalog
// (Azure OpenAI / Azure AI Foundry-Claude / Bedrock) via per-user CRUD. Global
...
async function loadProvidersPanel(){
  ...
  if(empty) empty.style.display='none';
  list.style.display='';
  list.innerHTML='<div style="color:var(--muted);padding:12px;font-size:13px">Loading…</div>';
  try{
    const data=await api('/api/models/config');
    const models=Array.isArray(data.models)?data.models:[];
    _lastModels=models;
    _modelProviderSchema=Array.isArray(data.providers)?data.providers:[];
    ...
    html+='<div class="settings-section-title" style="margin-top:18px">Global models <span style="color:var(--muted);font-weight:400;font-size:12px">🔒 read-only</span></div>';
    html+='<div style="display:flex;flex-direction:column;gap:6px;margin-top:6px">'+(globalModels.map(_renderModelRow).join('')||'<div style="color:var(--muted);font-size:12px">None.</div>')+'</div>';
    list.innerHTML=html;
    _bindAddModelForm();
...
function _providerLabel(pid){ const p=_modelProviderSchema.find(x=>x.id===pid); return p?p.label:(pid||''); }

function _renderModelRow(m){
  const editable=m.editable===true;
  const bits=[esc(_providerLabel(m.provider))];
  if(m.deployment) bits.push('deploy: '+esc(m.deployment));
...
function _renderAddModelForm(){
  const opts=_modelProviderSchema.map(p=>'<option value="'+esc(p.id)+'">'+esc(p.label)+'</option>').join('');
  return '<div id="addModelForm" style="border:1px solid var(--border);border-radius:10px;padding:12px;background:var(--card)">'
    +'<div style="font-weight:600;margin-bottom:8px">Add a model</div>'
    +'<label style="display:block;font-size:12px;color:var(--muted);margin-bottom:4px">Provider</label>'
    +'<select id="addModelProvider" class="settings-input" style="width:100%;margin-bottom:10px">'+opts+'</select>'
    +'<div id="addModelFields"></div>'
    +'<div id="addModelError" style="color:var(--error);font-size:12px;margin-top:6px;display:none"></div>'
    +'<button class="sm-btn" id="addModelBtn" type="button" style="margin-top:10px;width:100%;padding:8px;font-weight:600;color:var(--accent);border-color:rgba(233,69,96,.3)">Add model</button>'
    +'</div>';
}

function _renderProviderFields(pid){
  const p=_modelProviderSchema.find(x=>x.id===pid)||_modelProviderSchema[0];
  ...
  return p.fields.map(f=>{
    ...
    return '<div style="margin-bottom:8px"><label style="display:block;font-size:12px;color:var(--muted);margin-bottom:3px">'+esc(f.label)+req+'</label>'
      +'<input class="settings-input" data-field="'+esc(f.key)+'" type="'+type+'" autocomplete="off" spellcheck="false" placeholder="'+esc(f.placeholder||'')+'" style="width:100%"></div>';
  }).join('');
}

function _bindAddModelForm(){
  const sel=$('addModelProvider');
  if(!sel) return;
  const render=()=>{ const c=$('addModelFields'); if(c) c.innerHTML=_renderProviderFields(sel.value); const b=$('addModelBtn'); if(b) b.textContent='Add model'; };
  sel.onchange=render; render();
  const btn=$('addModelBtn'); if(btn) btn.onclick=saveProviderModel;
}

async function saveProviderModel(){
  const sel=$('addModelProvider'); const err=$('addModelError'); const btn=$('addModelBtn');
  if(!sel) return;
  const payload={provider:sel.value};
  document.querySelectorAll('#addModelFields [data-field]').forEach(inp=>{ const v=(inp.value||'').trim(); if(v) payload[inp.dataset.field]=v; });
...
async function deleteProviderModel(id){
  if(!id) return;
  if(!confirm('Delete model "'+id+'"?')) return;
  try{
    const res=await api('/api/models/config/delete',{method:'POST',body:JSON.stringify({id:id})});
    if(res&&res.ok){
      if(typeof showToast==='function') showToast('Model "'+id+'" deleted');
      await loadProvidersPanel();
      if(typeof populateModelDropdown==='function') populateModelDropdown();
    }else if(typeof showToast==='function') showToast((res&&res.error)||'Delete failed');
  }catch(e){ if(typeof showToast==='function') showToast(e.message||String(e)); }
}

// Edit = pre-fill the Add form with an existing user model. save_user_model upserts
// by id (and keeps the stored key/secret when the field is left blank), so saving
// over the same id is an in-place edit.
function editProviderModel(id){
  const m=(_lastModels||[]).find(x=>x.id===id && x.editable);
  if(!m){ if(typeof showToast==='function') showToast('Model not editable'); return; }
  const sel=$('addModelProvider'); if(!sel) return;
  sel.value=m.provider; sel.dispatchEvent(new Event('change',{bubbles:true}));
  const setF=(k,v)=>{ const el=document.querySelector('#addModelFields [data-field="'+k+'"]'); if(el && v!=null && v!=='') el.value=v; };
  setF('id', m.id); setF('deployment', m.deployment); setF('endpoint', m.endpoint);
  setF('api_version', m.api_version); setF('region', m.region); setF('aws_access_key_id', m.aws_access_key_id);
  if(m.max_tokens) setF('max_tokens', m.max_tokens);
  const idEl=document.querySelector('#addModelFields [data-field="id"]');
  if(idEl){ idEl.readOnly=true; idEl.style.opacity='0.7'; idEl.title='Model id cannot be changed when editing'; }
  const keyEl=document.querySelector('#addModelFields [data-field="api_key"]');
  if(keyEl && m.has_key){ keyEl.placeholder='•••• '+(m.api_key_masked||'')+' — leave blank to keep current key'; }
  const secEl=document.querySelector('#addModelFields [data-field="aws_secret_access_key"]');
  if(secEl && m.has_aws_secret){ secEl.placeholder='•••• leave blank to keep current'; }
  const btn=$('addModelBtn'); if(btn) btn.textContent='Save changes';
  const err=$('addModelError'); if(err){ err.style.display='none'; err.textContent=''; }
  const form=$('addModelForm'); if(form) form.scrollIntoView({behavior:'smooth',block:'start'});
  if(typeof showToast==='function') showToast('Editing "'+m.id+'" — change fields and Save');
}
...
function _applySavedSettingsUi(saved, body, opts){
  ...
  window._botName=body.bot_name||'joyjoy';
  if(typeof applyBotName==='function') applyBotName();
...
function loadMcpServers(){
  ...
  api('/api/mcp/servers').then(r=>{
    ...
    list.innerHTML=r.servers.map(s=>{
      ...
      const scope=s.scope==='user'?'user':'global';
      const scopeBadge=`<span class="mcp-transport-badge ${scope==='user'?'mcp-http':'mcp-unknown'}">${scope==='user'?'USER':'GLOBAL'}</span>`;
      const encodedName=encodeURIComponent(s.name).replace(/'/g,"\\'");
      // joyjoy: only per-user servers are writable; global servers are read-only.
      let controls;
      if(scope==='user'){
        controls=`<button type="button" class="mcp-toggle-btn ${isEnabled?'mcp-toggle-enabled':'mcp-toggle-disabled'}" title="${esc(t(isEnabled?'mcp_disable_server':'mcp_enable_server'))}" onclick="toggleMcpServer('${encodedName}',${!isEnabled})">${esc(t(isEnabled?'mcp_enabled_yes':'mcp_enabled_no'))}</button> <button type="button" class="mcp-toggle-btn mcp-toggle-disabled" title="Delete server" onclick="deleteMcpServer('${encodedName}')">✕ delete</button>`;
      } else {
        controls=`<span style="opacity:.6;font-size:11px">🔒 read-only</span>`;
      }
      return `<div class="mcp-server-row">
        ...
        <div class="mcp-server-meta"><span class="mcp-tool-count">${esc(t('mcp_tool_count',toolCount))}</span>${controls}</div>
      </div>`;
    }).join('');
  }).catch(()=>{list.innerHTML=`<div class="mcp-error-state" style="color:#ef4444;font-size:12px;padding:6px 0">${esc(t('mcp_load_failed'))}</div>`});
}
// ── MCP server create/delete (per-user; global servers are read-only) ──
...
function createMcpServer(){
  ...
  const transport=(($('mcpNewTransport')||{}).value)||'stdio';
  const detail=(($('mcpNewDetail')||{}).value||'').trim();
  if(!name||!detail){ showToast('Name and a command or URL are required','error'); return; }
  let cfg;
  if(transport==='http'){ cfg={url:detail}; }
  else { const parts=detail.split(/\s+/); cfg={command:parts[0], args:parts.slice(1)}; }
  api('/api/mcp/servers/'+encodeURIComponent(name),{method:'PUT', body:JSON.stringify(cfg)}).then(r=>{
    if(r&&r.ok){
      showToast('MCP server "'+name+'" saved');
      if($('mcpNewName')) $('mcpNewName').value='';
      if($('mcpNewDetail')) $('mcpNewDetail').value='';
      toggleMcpAddForm();
      _refreshMcpToolsetsCatalog();
...
function deleteMcpServer(encodedName){
  const name=decodeURIComponent(encodedName);
  if(typeof confirm==='function' && !confirm('Delete MCP server "'+name+'"?')) return;
  api('/api/mcp/servers/'+encodeURIComponent(name),{method:'DELETE'}).then(r=>{
    if(r&&r.ok){ showToast('MCP server "'+name+'" deleted'); _refreshMcpToolsetsCatalog(); loadMcpServers(); loadMcpTools(); }
...
switchSettingsSection=function(name, opts){
  ...
  // MCP servers/tools moved to their own left-nav panel (loaded by switchPanel('mcp')).
