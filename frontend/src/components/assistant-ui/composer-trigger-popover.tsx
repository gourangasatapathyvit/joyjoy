						...
						{items.map((item, index) => {
							...
							return (
								<ComposerPrimitive.Unstable_TriggerPopoverItem
									...
									item={item}
									index={index}
									className="hover:bg-accent focus:bg-accent data-[highlighted]:bg-accent flex w-full cursor-pointer flex-col items-start gap-0.5 px-2.5 py-1.5 text-start transition-colors outline-none"
								>
									...
									{item.description && (
										<span className="text-muted-foreground ms-5.5 line-clamp-2 text-[11px] leading-tight">
											{item.description}
										</span>
									)}
								</ComposerPrimitive.Unstable_TriggerPopoverItem>
...
const ComposerTriggerPopoverImpl: FC<ComposerTriggerPopoverProps> = ({
	...
	return (
		<ComposerPrimitive.Unstable_TriggerPopover
			...
			className={cn(
				"aui-composer-trigger-popover bg-popover text-popover-foreground absolute start-0 bottom-full z-50 mb-2 max-h-56 w-60 overflow-y-auto rounded-lg border shadow-lg",
				className,
			)}
			{...props}
		>
