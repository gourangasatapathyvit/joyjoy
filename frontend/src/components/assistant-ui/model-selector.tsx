...
// Adapted from assistant-ui's Model Selector registry component
// (https://www.assistant-ui.com/docs/ui/model-selector). The upstream
// `ModelSelector` wrapper registers the selection with assistant-ui's
// ModelContext via `useAui`; joyjoy instead drives model + reasoning effort
// through the chat store (sent on each run), so that auto-registration and the
// `useAui` import are removed. Everything else — Root/Trigger/Value/Content/
// Search/List/Group/Item/Effort, controllable state, cmdk search — is intact.
...
function ModelSelectorEffort({
	...
	return (
		<div
			...
			className={cn(
				"flex flex-wrap items-center justify-between gap-3 border-t px-3 py-2",
				className,
			)}
