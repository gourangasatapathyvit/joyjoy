...
- **Providers — fetch-and-pick models** (`routes/ProvidersPanel.tsx`): instead of hand-typing a model id, enter credentials and "Fetch models" to browse the provider's live catalog (`useDiscoverModels`), then multi-select to add (Add dialog) or pick one to switch a model's deployment (Edit dialog, reusing the model's own stored credentials). A manual-entry fallback covers ids the API doesn't list. xAI is the one provider with two auth modes: a "Use OAuth login instead" toggle on its single "xAI (Grok)" dropdown entry swaps the credential form for a device-code login widget in place, rather than listing `xai`/`xai_oauth` as two separate rows.
- **Not every `@/components/ui/*` primitive is Radix.** `collapsible`, `switch`, and `tooltip` wrap **base-ui** (`@base-ui/react`), which uses presence-only state attributes (`data-panel-open`, `data-open`/`data-closed`, `data-starting-style`/`data-ending-style`) instead of Radix's `data-state="open"|"closed"` — a couple of older components (`popover.tsx`) are still genuinely Radix. A Tailwind selector written against the wrong convention doesn't error, it just never matches (see `tool-group.tsx`/`tool-fallback.tsx`/`reasoning.tsx` for the fixed version). Check which library a component actually imports before copying a Radix-style selector onto it.
